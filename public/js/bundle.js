...
const displayMap = locations => {
  ...
  locations.forEach(location => {
    ...
    L.marker(coords, {
      icon: customIcon
    }).addTo(map).bindPopup("<p>Day ".concat(location.day, ": ").concat(location.description, "</p>"), {
      autoClose: false,
      closeOnClick: false,
...
if (userPasswordForm) {
  userPasswordForm.addEventListener('submit', async e => {
    ...
    await (0, _updateUserSettings.updateSettings)({
      passwordCurrent,
      password,
      passwordConfirm
    }, 'password');
